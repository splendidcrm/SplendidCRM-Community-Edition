									...
									<tr>
										<td className='ModuleActionsInnerCell'>
										{
											...
											this.tabsSecondary.map((tabMenu) => (
												<a href={ Credentials.RemoteServer + 'React/' + tabMenu.MODULE_NAME } className='otherTabMoreLink' style={ {minWidth: '10rem'} } onClick={ (e) => { e.preventDefault(); this._onModuleClick(tabMenu.MODULE_NAME); } }>{ L10n.Term(tabMenu.DISPLAY_NAME) }</a>
											))
										}
										</td>
									</tr>
										...
										<tbody><tr>
											<td className='ModuleActionsInnerCell'>
												<div className='ModuleActionsInnerHeader' style={ {fontWeight: 'bold'} }>{ L10n.Term('.LBL_ACTIONS') }</div>
												{
													...
													(
														<a href={ Credentials.RemoteServer + 'React/' + item.key } key={ 'action_' + item.key } className='ModuleActionsMenuItems' onClick={(e) => this._onAction(item.MODULE_NAME, item)}>{ item.label }</a>
													))
												}
											</td>
											<td className='ModuleActionsInnerCell'>
						...
						>
							<tr>
								...
								<td className={ (tabMenu.MODULE_NAME == activeModule ? 'current' : 'other') + 'Tab' } style={ {whiteSpace: 'nowrap', verticalAlign: 'middle'} }>
									<a href={ Credentials.RemoteServer + 'React/' + tabMenu.MODULE_NAME } onClick={ (e) => { e.preventDefault(); this._onModuleClick(tabMenu.MODULE_NAME); } } className={ (tabMenu.MODULE_NAME == activeModule ? 'current' : 'other') + 'TabLink' }>{ L10n.Term(tabMenu.DISPLAY_NAME) }</a>
								</td>
