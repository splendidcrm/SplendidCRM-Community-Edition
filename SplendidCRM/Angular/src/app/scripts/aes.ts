...
export default class Aes
{
	...
	{    // main Cipher function [§5.1]
		...
		let state: any[] = [[], [], [], []];  // initialise 4xNb byte-array 'state' with input [§3.4]
	...
	static subBytes( s: any, Nb: number )
	...
	static shiftRows( s: any, Nb: number )
	...
	static mixColumns( s: any, Nb: number )
	...
	static addRoundKey( state: any, w: any, rnd: number, Nb: number )
	...
	static sBox = [0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
	...
	static rCon = [[0x00, 0x00, 0x00, 0x00],
	[0x01, 0x00, 0x00, 0x00],
	[0x02, 0x00, 0x00, 0x00],
	[0x04, 0x00, 0x00, 0x00],
	[0x08, 0x00, 0x00, 0x00],
	...
	static Ctr = class // Aes.Ctr namespace: a subclass or extension of Aes
	{
		/** 
		 ...
		 */
		static decrypt = function ( ciphertext: any, password: string, nBits: number )
		{
			var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
			if ( !( nBits == 128 || nBits == 192 || nBits == 256 ) ) return '';  // standard allows 128/192/256 bit keys
...
class Base64
{
	 ...
	 */
	static encode( str: any, utf8encode?: any)
	 ...
	 * @returns {String} decoded string
	 */
	static decode( str: any, utf8decode?: any)
	{
