				...
				if ( !IsPostBack )
				{
								...
								using ( DbDataAdapter da = dbf.CreateDataAdapter() )
								{
									...
									using ( DataTable dtCurrent = new DataTable() )
									{
										...
										if ( dtCurrent.Rows.Count > 0 && (SplendidCRM.Security.GetRecordAccess(dtCurrent.Rows[0], m_sMODULE, "edit", "ASSIGNED_USER_ID") >= 0) )
										{
											...
											{
												// 01/05/2020 Paul Enlivant.  We need to use FindByValue to locate the item. 
												lstPARENT_TYPE.Items.Remove(lstPARENT_TYPE.Items.FindByValue("Contacts"));
											}
					...
					else
					{
						...
						if ( !Sql.IsEmptyGuid(gCONTACT_ID) )
						{
							...
							// 11/30/2017 Paul.  Add ASSIGNED_SET_ID for Dynamic User Assignment. 
							Guid   gASSIGNED_SET_ID  = Guid.Empty;
							// 04/20/2018 Paul.  Specify sCONTACT_NAME instead of sCONTACT_TYPE twice. 
							SqlProcs.spPARENT_GetWithTeam(ref gCONTACT_ID, ref sMODULE, ref sCONTACT_TYPE, ref sCONTACT_NAME, ref gASSIGNED_USER_ID, ref sASSIGNED_TO, ref sASSIGNED_TO_NAME, ref gTEAM_ID, ref sTEAM_NAME, ref gTEAM_SET_ID, ref gASSIGNED_SET_ID);
							if ( !Sql.IsEmptyGuid(gCONTACT_ID) )
							{
