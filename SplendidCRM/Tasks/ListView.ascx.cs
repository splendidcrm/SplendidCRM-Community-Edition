...
namespace SplendidCRM.Tasks
{
	...
	public class ListView : SplendidControl
	{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spTASKS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.STATUS, T10n.ToServerTime(ctlMassUpdate.DATE_DUE), T10n.ToServerTime(ctlMassUpdate.DATE_START), ctlMassUpdate.PRIORITY, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, String.Empty, false, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
		...
		protected void grdMain_OnSelectMethod(int nCurrentPageIndex, int nPageSize)
		{
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				using ( IDbCommand cmd = con.CreateCommand() )
				{
					...
					// 11/01/2017 Paul.  Use a module-based flag so that Record Level Security is only enabled when needed. 
					// 04/27/2020 Paul.  CAN_CLOSE is not available in Archive view.  Create dummy field. 
					cmd.CommandText = "select " + (Request.Form[ctlExportHeader.ExportUniqueID] != null ? SplendidDynamic.ExportGridColumns(m_sMODULE + ".Export", arrSelectFields) : Sql.FormatSelectFields(arrSelectFields))
					                ...
					                + (ArchiveViewEnabled() ? "     , null as CAN_CLOSE" + ControlChars.CrLf : String.Empty)
					                + cmd.CommandText;
					if ( nPageSize > 0 )
					{
		...
		private void InitializeComponent()
		{
			...
			// 04/27/2020 Paul.  CAN_CLOSE is not available in Archive view. 
