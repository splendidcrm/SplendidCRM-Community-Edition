...
namespace SplendidCRM.Emails
{
	...
	public class ListView : SplendidControl
	{
								...
								using ( IDbTransaction trn = Sql.BeginTransaction(con) )
								{
									try
									{
										while ( stk.Count > 0 )
										{
											...
											SqlProcs.spEMAILS_MassUpdate(sIDs, ctlMassUpdate.ASSIGNED_USER_ID, ctlMassUpdate.PRIMARY_TEAM_ID, ctlMassUpdate.TEAM_SET_LIST, ctlMassUpdate.ADD_TEAM_SET, String.Empty, false, trn);
										}
										trn.Commit();
									}
									catch(Exception ex)
		...
		private void InitializeComponent()
		{
			...
			{
				arrSelectFields.Add("'.dom_email_types.' + TYPE as TYPE_TERM");
				arrSelectFields.Add("0 as ATTACHMENT_COUNT");
			}
			else
			{
			...
			if ( ArchiveViewEnabled() )
			{
				arrSelectFields.Remove("TYPE_TERM");
				arrSelectFields.Remove("ATTACHMENT_COUNT");
			}
